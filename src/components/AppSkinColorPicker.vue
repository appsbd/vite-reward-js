<template>
    <div class="app-color-skin" :style="'justify-content:'+align+';'">
      <div class="color-picker-item"  :key="color.name+'_'+index" v-for="(color,index) in colors">
        <input :checked="color.name==modelValue" type="radio"  :value="color.name" :name="name" :id="name+'-'+index" @input="updateValue"  />
        <label :for="name+'-'+index" :title="color?.title" :style="'background:'+color?.color" >
