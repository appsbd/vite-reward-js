import {confirmed, email, min,max,max_value,min_value, required,url,numeric} from "@vee-validate/rules";
    ...
    install(app,translate) {
        ...
        const allRules={
            ...
            min_value:(value,params,field) => {
                return min_value(value,params)?true:translateGettext("%{fld_name} should be more than "+ params[0],{fld_name:getFieldLabel(field)});
            },
            max:(value,params,field) => {
                return max(value,params);
            },
            max_value:(value,params,field) => {
                return max_value(value,params)?true:translateGettext("%{fld_name} should be less than "+ params[0],{fld_name:getFieldLabel(field)});;
            },
